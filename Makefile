...
console-dev:
	_build/dev/rel/chat/bin/chat console

rel-prod:
	$(SED) -i 's/{chat, "chat-version"}/{chat, "$(VER)"}/g' ./rebar.config
	$(PWD)/script/rebar3 as prod release
	$(PWD)/script/rebar3 as prod tar
	$(SED) -i 's/{chat, "$(VER)"}/{chat, "chat-version"}/g' ./rebar.config
    #$(SCP) -P 8522 $(PWD)/_build/prod/rel/chat/chat-$(VER).tar.gz $(FS)
	@printf "\nApplication: %s\n" $(PWD)/_build/prod/rel/chat/chat-$(VER).tar.gz
...
rel-stage:
	...
	$(SED) -i 's/{chat, "$(VER)"}/{chat, "chat-version"}/g' ./rebar.config
    #$(SCP) -P 8522 $(PWD)/_build/stage/rel/chat/chat-$(VER).tar.gz $(FS)
	@printf "\nApplication: %s\n" $(PWD)/_build/stage/rel/chat/chat-$(VER).tar.gz
